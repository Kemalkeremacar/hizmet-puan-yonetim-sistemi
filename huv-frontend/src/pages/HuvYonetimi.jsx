...
// ============================================
// Güncellenen Satır Bileşeni (Expandable)
// ============================================
function VersiyonGuncellenenRow({ item }) {
  const [expanded, setExpanded] = useState(false);

  const degisiklikler = [];
  if (item.BirimDegisti) degisiklikler.push({ field: 'Birim', color: 'warning' });
  if (item.IslemAdiDegisti) degisiklikler.push({ field: 'İşlem Adı', color: 'info' });
  if (item.SutKoduDegisti) degisiklikler.push({ field: 'SUT Kodu', color: 'secondary' });
  if (item.NotDegisti) degisiklikler.push({ field: 'Not', color: 'default' });

  const hasNonBirimChanges = item.IslemAdiDegisti || item.SutKoduDegisti || item.NotDegisti;
  const fark = item.YeniBirim != null && item.EskiBirim != null ? item.YeniBirim - item.EskiBirim : null;

  const formatTL = (v) => v != null ? `₺${parseFloat(v).toLocaleString('tr-TR', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}` : '-';
  ...
  return (
    <>
      <TableRow
        hover
        onClick={() => hasNonBirimChanges && setExpanded(!expanded)}
        sx={{ cursor: hasNonBirimChanges ? 'pointer' : 'default' }}
      >
        <TableCell sx={{ pr: 0 }}>
          {hasNonBirimChanges && (
          ...
          {item.BirimDegisti ? (
            <Stack direction="row" spacing={0.5} alignItems="center" justifyContent="flex-end">
              <Typography variant="body2" color="text.secondary" sx={{ textDecoration: 'line-through' }}>
                {formatTL(item.EskiBirim)}
              </Typography>
              <SwapIcon sx={{ fontSize: 14, color: 'text.disabled' }} />
              <Typography variant="body2" fontWeight="600">
                {formatTL(item.YeniBirim)}
              </Typography>
            </Stack>
          ) : (
            <Typography variant="body2" color="text.secondary">{formatTL(item.EskiBirim || item.YeniBirim)}</Typography>
          )}
        </TableCell>
        <TableCell align="center">
          {fark !== null && item.BirimDegisti ? (
            <Chip
            ...
            !item.BirimDegisti && <Chip label="—" size="small" variant="outlined" sx={{ fontSize: '0.7rem' }} />
          )}
        </TableCell>
      </TableRow>
      {hasNonBirimChanges && (
            ...
            <Collapse in={expanded} timeout="auto" unmountOnExit>
              <Box sx={{ py: 1.5, px: 2, my: 1, bgcolor: '#f8f9fa', borderRadius: 1, borderLeft: '3px solid', borderLeftColor: 'info.main' }}>
                ...
                <Stack spacing={1}>
                  ...
                  {item.SutKoduDegisti && (
                    <Box>
                      <Typography variant="caption" fontWeight="600" color="text.secondary">SUT Kodu:</Typography>
                      <Stack direction="row" spacing={1} alignItems="center" sx={{ mt: 0.25 }}>
                        <Typography variant="caption" sx={{ color: 'error.main', textDecoration: 'line-through' }}>
                          {item.EskiSutKodu || '(boş)'}
                        </Typography>
                        <SwapIcon sx={{ fontSize: 12, color: 'text.disabled' }} />
                        <Typography variant="caption" fontWeight="600" color="success.dark">
                          {item.SutKodu || '(boş)'}
                  ...
                  {item.NotDegisti && (
                    <Box>
                      <Typography variant="caption" fontWeight="600" color="text.secondary">Not:</Typography>
                      <Stack spacing={0.5} sx={{ mt: 0.5 }}>
                        ...
                        <Box sx={{ p: 1, bgcolor: '#f0fff0', borderRadius: 0.5 }}>
                          <Typography variant="caption" color="success.dark" sx={{ whiteSpace: 'pre-wrap', wordBreak: 'break-word' }}>
                            {truncate(item.Not || item.YeniNot, 200) || '(boş)'}
                          </Typography>
                        </Box>
                      </Stack>
                    </Box>
                    ...
                    <Table size="small" stickyHeader>
                      ...
                      <TableBody>
                        {versiyonDetay.guncellenenler.map((item, idx) => (
                          <VersiyonGuncellenenRow key={idx} item={item} />
                        ))}
                      </TableBody>
                    </Table>
                  </TableContainer>
